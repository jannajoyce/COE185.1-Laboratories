...
*/
static void lcd_writenibble(unsigned char lcdbits) {
...
//// LAB 3 !!!!!!
/*
  lcd_moveto - Move the cursor to a specified position on the LCD.
*/
void lcd_moveto(unsigned char row, unsigned char col) {
    ...
    switch (row) {
        ...
        case 2: address = 0x14 + col; break;  // Third line (if applicable)
        case 3: address = 0x54 + col; break;  // Fourth line (if applicable)
...
/*
  lcd_stringout - Output a string of characters to the LCD.
*/
void lcd_stringout(char *str) {
    while (*str) {
        lcd_writedata(*str++);  // Send each character to the data register
    }
}
...
*/
void lcd_clear(void) {
    lcd_writecommand(LCD_CLR);  // Send the clear command to the LCD
    _delay_ms(2);               // Delay to allow the LCD to clear
}
